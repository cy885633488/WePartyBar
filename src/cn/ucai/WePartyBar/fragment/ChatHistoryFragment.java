...
import cn.ucai.WePartyBar.adapter.ChatHistoryAdapter;
...
public class ChatHistoryFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		return inflater.inflate(cn.ucai.WePartyBar.R.layout.fragment_conversation_history, container, false);
	}
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		errorText = (TextView) errorItem.findViewById(cn.ucai.WePartyBar.R.id.tv_connect_errormsg);
		// contact list
		contactList = ((DemoHXSDKHelper)HXSDKHelper.getInstance()).getContactList();
		listView = (ListView) getView().findViewById(cn.ucai.WePartyBar.R.id.list);
		adapter = new ChatHistoryAdapter(getActivity(), 1, loadUsersWithRecentChat());
		// 设置adapter
		listView.setAdapter(adapter);
		final String st = getResources().getString(cn.ucai.WePartyBar.R.string.Cant_chat_with_yourself);
		listView.setOnItemClickListener(new OnItemClickListener() {

			@Override
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				EMContact emContact = adapter.getItem(position);
				if (adapter.getItem(position).getUsername().equals(WePartyBarApplication.getInstance().getUserName()))
		...
		// 搜索框
		query = (EditText) getView().findViewById(cn.ucai.WePartyBar.R.id.query);
		// 搜索框中清除button
		clearSearch = (ImageButton) getView().findViewById(cn.ucai.WePartyBar.R.id.search_clear);
	...
	public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
		...
		getActivity().getMenuInflater().inflate(cn.ucai.WePartyBar.R.menu.delete_message, menu);
		// }
	}

	@Override
	public boolean onContextItemSelected(MenuItem item) {
		if (item.getItemId() == cn.ucai.WePartyBar.R.id.delete_message) {
	 ...
	 */
	public void refresh() {
		adapter = new ChatHistoryAdapter(getActivity(), cn.ucai.WePartyBar.R.layout.row_chat_history, loadUsersWithRecentChat());
		listView.setAdapter(adapter);
		adapter.notifyDataSetChanged();
	}
